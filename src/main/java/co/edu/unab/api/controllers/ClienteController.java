...
public class ClienteController {
    ...
    public ArrayList<ClienteModel> obtenerClientePorUbicacion(@PathVariable("ciudad")String ciudad,@PathVariable("departamento")String departamento){
        return clienteService.clientesPorCiudad(ciudad, departamento); 
    }
    @GetMapping(path = "/puntos/{puntos}")
    public ArrayList<ClienteModel> obtenerClientePorMayorPuntos(@PathVariable("puntos")long puntos){
        return clienteService.clientesMayorOIgualPuntos(puntos); 
    }
    @GetMapping(path = "/puntosm/{puntos}")
    public ArrayList<ClienteModel> obtenerClientePorMenorPuntos(@PathVariable("puntos")long puntos){
        return clienteService.clientesMenorOIgualPuntos(puntos);
    }
    @GetMapping(path = "/id/{apellido}/{nombre}")
    public ArrayList<ClienteModel> obtenerIdPorApellido(@PathVariable("apellido")String apellido,@PathVariable("nombre")String nombre){
        return clienteService.obtenerIdClientes(apellido,nombre);
    }
}
